...
class SubtractAgent(BaseCalculationAgent):
    def __init__(self):
        sys_msg = """
            You are a specialist in Maths calculations.
            From the given query identify the 2 numbers that need to be subtracted and output first_mumber and second_number.
...
@server.agent()
async def subtract_agent_service(input: list[Message]) -> AsyncGenerator[RunYield, RunYieldResume]:    
       
        workflow = SubtractAgent().workflow
